...
# ---------------- Full Model ----------------
class DASNet1_EncoderDecoder_CBAM(nn.Module):
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        # ----- encoder CBAM -----
        self.cbam_e1 = CBAM(128)
        self.cbam_e2 = CBAM(256)
        self.cbam_e3 = CBAM(512)
        self.cbam_e4 = CBAM(1024)
        ...
        # ----- decoder CBAM -----
        self.cbam_d1 = CBAM(256)
        self.cbam_d2 = CBAM(128)
        self.cbam_d3 = CBAM(64)
        self.cbam_d4 = CBAM(32)
    ...
    def forward(self, x):
        ...
        x = self.encoder.denseblock1(x)
        skip1 = x
        x = self.encoder.transition1(x)
        x = self.cbam_e1(x)

        x = self.encoder.denseblock2(x)
        skip2 = x
        x = self.encoder.transition2(x)
        x = self.cbam_e2(x)

        x = self.encoder.denseblock3(x)
        skip3 = x
        x = self.encoder.transition3(x)
        x = self.cbam_e3(x)

        x = self.encoder.denseblock4(x)
        x = self.cbam_e4(x)
        skip4 = x
        ...
        # ----- Decoder: Concat -> Conv -> CBAM -> Upsample -----
...
if __name__ == '__main__':
    net = DASNet1_EncoderDecoder_CBAM(num_classes=2)
