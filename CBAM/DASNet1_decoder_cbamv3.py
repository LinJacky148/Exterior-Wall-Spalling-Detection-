...
# --------------------- Full Model ---------------------
class DASNet1_CBAM_V3(nn.Module):
    ...
    def forward(self, x):
        ...
        # ----- Decoder V3: Concat -> 3x3 Conv -> Upsample -> CBAM -----
        x = torch.cat([x_aspp, skips[-1]], dim=1)   # Concat
        x = self.decode1_conv(x)                    # 3x3 Conv
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)  # Upsample
        x = self.cbam1(x)                           # CBAM

        x = torch.cat([x, skips[-2]], dim=1)        # Concat
        x = self.decode2_conv(x)                    # 3x3 Conv
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)  # Upsample
        x = self.cbam2(x)                           # CBAM

        x = torch.cat([x, skips[-3]], dim=1)        # Concat
        x = self.decode3_conv(x)                    # 3x3 Conv
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)  # Upsample
        x = self.cbam3(x)                           # CBAM

        x = torch.cat([x, skips[-4]], dim=1)        # Concat
        x = self.decode4_conv(x)                    # 3x3 Conv
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)  # Upsample
        x = self.cbam4(x)                           # CBAM
...
if __name__ == '__main__':
    net = DASNet1_CBAM_V3(num_classes=2)
